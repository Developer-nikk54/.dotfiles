// Cursor trail shader with fixed color #FF70BF
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Fixed exact color: #FF70BF
    // vec3 srgb = vec3(1.0, 0.439, 0.749);
    // vec3 trailColor = pow(srgb, vec3(2.2));
    // vec4 enhancedColor = vec4(trailColor, 1.0);
    //
    ...
    // Fixed exact color: #D4D4D4
    vec3 srgb = vec3(0.831373, 0.831373, 0.831373);
    vec3 trailColor = pow(srgb, vec3(2.2));
    vec4 enhancedColor = vec4(trailColor, 1.0);
